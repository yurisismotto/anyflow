...
import io.github.yurisismotto.anyflow.clipboard.ClipboardReadFailed
...
class ClipboardInstrumentedTest {
    ...
    /**
     * Runs [body] with AnyFlow on screen **and holding window focus**.
     *
    ...
    private fun withFocusedActivity(body: (MainActivity) -> Unit) {
        ...
        assertTrue(
            "AnyFlow never took window focus in $FOCUS_ATTEMPTS attempts of " +
    ...
    fun setPrimaryClip_applies_a_remote_clip() {
        ...
        // clip from a computer must land while AnyFlow is in the background.
     ...
     * something the person did not copy. Nothing in AnyFlow depends on it.
     */
    @Test
    ...
    companion object {
        /** `adb logcat -s AnyFlowClipTest` collects every observation above. */
        private const val TAG = "AnyFlowClipTest"

        /**
         * How long to wait for AnyFlow to take window focus.
         *
