...
/**
 * Finds Fedroid Bridge daemons on the local network via DNS-SD.
 *
...
class Discovery(context: Context) {
    ...
    fun browse(): Flow<Found> = callbackFlow {
        val lock = wifi.createMulticastLock("fedroid-bridge-discovery").apply {
        ...
        val resolveListener = object : NsdManager.ResolveListener {
            ...
            override fun onServiceResolved(info: NsdServiceInfo) {
                ...
                trySend(
                    Found(
                        deviceId = attributes["id"]?.toString(Charsets.UTF_8)
                            ?.takeIf { it.length <= 64 && it.all { c -> c.isHex() } },
                        deviceName = attributes["dn"]?.toString(Charsets.UTF_8)
                            ?.let { name -> name.filter { !it.isISOControl() }.take(64) },
                        address = InetSocketAddress(host, info.port),
                    ),
                )
            }
        }
    ...
    companion object {
        ...
        const val SERVICE_TYPE = "_fedroid-bridge._tcp."
        private const val SERVICE_TYPE_SHORT = "_fedroid-bridge"
    }
}
