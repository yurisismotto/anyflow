package dev.fedroid.bridge.pairing
...
/**
 * The pairing proof, byte-for-byte identical to `fedroid_core::pairing`.
 *
 * ```text
 * proof = HMAC-SHA256(
 *     key = token,
...
object PairingProof {
    ...
    private val PROOF_DOMAIN = "fedroid-bridge/pairing-proof/v1".toByteArray(Charsets.US_ASCII)
    private val CONFIRM_DOMAIN =
        "fedroid-bridge/pairing-confirm/v1".toByteArray(Charsets.US_ASCII)
