 ...
 * with a `RELATIVE_PATH` of `Download/OmniBridge` needs **no storage permission
 * at all** on API 29+, which is this app's floor. The user finds the file in
 * Files under Downloads/OmniBridge, and it survives uninstalling the app.
 *
...
class Downloads(private val context: Context) {
    ...
    private val relativePath = "${Environment.DIRECTORY_DOWNLOADS}/OmniBridge"
