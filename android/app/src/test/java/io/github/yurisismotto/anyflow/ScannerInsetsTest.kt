...
import io.github.yurisismotto.anyflow.ui.ScannerInsets
import java.io.File
...
class ScannerInsetsTest {
    ...
    fun `the inset rule hard-codes no sizes`() {
        val code = withoutKotlinComments(
            File("src/main/java/io/github/yurisismotto/anyflow/ui/ScannerInsets.kt").readText(),
        )
    ...
    @Test
    fun `no AnyFlow theme turns on a fullscreen or translucent bar flag`() {
    ...
    @Test
    fun `AnyFlow's capture activity adds inset handling and nothing else`() {
        val code = File(
            "src/main/java/io/github/yurisismotto/anyflow/ui/PairingCaptureActivity.kt",
        ).readText()
