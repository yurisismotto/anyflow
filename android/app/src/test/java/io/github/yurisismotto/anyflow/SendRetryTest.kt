...
import io.github.yurisismotto.anyflow.files.StreamAuth
...
class SendRetryTest {
    ...
    private val debian = peer("anyflow-d13", 2, setOf(FilesCapability.ID))
    ...
    fun `a retry failure never puts a content uri on screen`() {
        ...
        assertEquals("AnyFlow could not send that file.", message)
