...
//!
//! The policy *type* lives in [`anyflow_core::notification_policy`], because
...
pub use anyflow_core::notification_policy::{LockPolicy, NotificationPolicy};
...
pub trait NotificationAuthorizer: Send + Sync {
    ...
    async fn policy_for(&self, peer: &anyflow_core::Fingerprint) -> NotificationPolicy;
}
...
impl NotificationAuthorizer for DenyAll {
    async fn policy_for(&self, _peer: &anyflow_core::Fingerprint) -> NotificationPolicy {
