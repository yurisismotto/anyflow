...
use fedroid_core::framing;
use fedroid_core::session::PROTOCOL_VERSION_MAX;
...
async fn an_unknown_peer_cannot_send_anything_but_a_pair_request() {
    ...
    assert_eq!(ack.pairing_nonce.len(), fedroid_core::pairing::NONCE_LEN);
...
async fn a_client_without_a_certificate_is_rejected_during_the_handshake() {
    ...
    let verifier = fedroid_core::tls::PinnedServerCertVerifier::new(server.fingerprint);
    ...
    let tcp = tokio::net::TcpStream::connect(server.addr)
        .await
        .expect("tcp");
    let name = rustls_pki_types::ServerName::try_from("fedroid.invalid").expect("name");
