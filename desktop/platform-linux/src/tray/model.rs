...
//! implementation is wrong: it makes every action AnyFlow's GUI will ever
...
///
/// The same string as `APP_ID` in `anyflow-gui`, the desktop entry's
...
pub const DESKTOP_APP_ID: &str = "io.github.yurisismotto.anyflow";
...
/// wrong.
pub const DESKTOP_APP_OBJECT_PATH: &str = "/io/github/yurisismotto/anyflow";
...
/// Flow A from `docs/design/assets/app-icon.svg`. A tray item that carried an
...
pub const ITEM_TITLE: &str = "AnyFlow";
...
/// — `anyflowd` genuinely is a background service — but the category
...
///   respond by unhiding it and, on Plasma, animating it. AnyFlow's phone
...
///   permission to hide the item entirely. A running AnyFlow is a running
...
/// The tooltip title.
pub const TOOLTIP_TITLE: &str = "AnyFlow";
...
/// in the session, at any time, without a click. The tray is the one AnyFlow
...
impl TrayAction {
    ...
    ///
    /// These three names are asserted against `anyflow-gui`'s own public
...
/// deep in a menu that is drawn by another process. "Quit AnyFlow" is absent
/// for a different reason: the only thing it could honestly quit is
/// `anyflowd`, and stopping the continuity service from a tray menu is not
