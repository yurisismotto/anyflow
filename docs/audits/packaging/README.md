...
| [Fedora integration](PACKAGING-V1-FEDORA-INTEGRATION.md) | the RPM as a real installed desktop product: subpackage split, desktop metadata, lifecycle macros, firewall, `%doc` trim | P7, P5, R10, Q3 closed; one flaky `%check` test carried forward |
...
| `packaging/tests/install-smoke.sh` | install, remove, reinstall and — above all — that no transaction touches the user's trust store |
