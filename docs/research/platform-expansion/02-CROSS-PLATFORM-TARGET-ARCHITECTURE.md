...
| --- | --- | --- |
| Wire format, framing, envelope | Silent corruption or a hang between two AnyFlow devices | **Share** |
...
The list of "share" items is exactly the set that is already in `anyflow-core` and the
...
```
        ...
        ┌─────────────────────┴──────────────────────┐
        │                                             │
   anyflow-proto (Rust)                        Kotlin protos (Android)
        │
   ┌────┴──────────────────────────────────────────────────┐
   │  anyflow-core            PORTABLE, no_std-ish policy:  │
   ...
   └────┬────────────────────────────────────────┘
        │
   ┌────┴──────────────────────────────────────┐
   │  anyflow-runtime  (NEW; today's daemon     │
   │  minus its Linux assumptions)              │
   │    listener  mdns  state  SessionHost impl │
   │    ── ControlTransport trait      (NEW)    │
   └────┬───────────────┬───────────────┬───────┘
        │               │               │
  anyflow-linux    anyflow-windows   anyflow-apple      ← platform adapter crates
...
| **Δ3** | `anyflow-daemon` splits into `anyflow-runtime` (portable) + `anyflow-linux` (adapter); control-protocol *types* move to their own crate | Lets the GUI and future UIs depend on types without inheriting a platform |
...
summary: AnyFlow's value is *deep OS integration* (clipboard ownership, tray/menu-bar
...
**Should the Rust core be a library?** It already is. `anyflow-core` is a library with no
...
`anyflow-runtime` **library**, hosted by a per-platform process with a per-platform
lifetime. The abstract name for that process is defined in
[17](17-BACKGROUND-EXECUTION-MODEL.md) as the **AnyFlow Agent**.

**Should `anyflow-core` be separated from `anyflow-linux`?** Yes, but the split that matters
is not `core`/`linux` — core is already almost clean. It is
`anyflow-daemon` → `anyflow-runtime` + `anyflow-linux`, plus extracting the control-protocol
...
pub trait IdentitySigner: Send + Sync + std::fmt::Debug {
    ...
    /// Honest self-description for the UI and for `anyflow status`.
...
2. **`sign()` is synchronous and blocking**, because rustls's `Signer::sign` is. A Secure
   ...
   handshake. Therefore AnyFlow's Apple identity key must be created **without**
