...
| ARCH-003 | Split `anyflow-daemon`; extract control types | P0 | 0 | refactor | ARCH-001 | `anyflow-runtime` (portable) + `anyflow-linux`; a `ControlTransport` trait; control request/response types in their own crate so the GUI stops depending on the daemon | `daemon/tests/control.rs` passes unmodified; the GUI builds without `anyflow-daemon` | [01 §4](01-CURRENT-ARCHITECTURE-AUDIT.md), [17](17-BACKGROUND-EXECUTION-MODEL.md) |
...
| ARCH-008 | Document the AnyFlow Agent concept | P2 | X | docs | — | Add the Agent abstraction and its per-platform lifetimes to `docs/architecture/` | A reader understands why Windows is not a Service and why iOS has no agent | [17](17-BACKGROUND-EXECUTION-MODEL.md) |
...
| LINUX-004 | firewalld default-block diagnosis | P2 | 1 | feature | Fedora blocks inbound 55432 by default; Debian and Ubuntu do not. The reference platform is the one that fails | `anyflow status` detects an unreachable listener and explains it | [07 §7](07-LINUX-PACKAGING.md) |
| LINUX-005 | XDG autostart entry | P1 | 1 | packaging | Covers non-systemd distributions and users who never run `systemctl --user enable` | AnyFlow starts at login on a systemd and a non-systemd distribution | [04 §8](04-LINUX-PORTABILITY.md) |
...
| LINUX-007 | Linux TPM2 / PKCS#11 signer | P2 | X | security | Otherwise Linux becomes the only AnyFlow platform without hardware-backed identity | A handshake with a TPM-held key, as an unprivileged user, with no manual sysadmin step | [14 §5.3](14-CROSS-PLATFORM-IDENTITY-AND-KEY-STORAGE.md) |
| LINUX-008 | Linux share-target integration | P3 | X | feature | The desktop with the deepest integration elsewhere has the weakest share story | "Send to AnyFlow" appears in a file manager | [16 §7](16-CROSS-PLATFORM-FILES.md) |
...
| KDE-002 | Document the Klipper clipboard↔selection hazard | P2 | 3 | docs | With that Klipper setting on, every mouse selection becomes a clipboard event and is synced. AnyFlow is behaving correctly; the desktop changed what CLIPBOARD means | A documented note, surfaced in `clipboard status` on Plasma | [06 §5](06-KDE-PLASMA-WAYLAND.md) |
...
| ID | Title | Pri | Wave | Type | Description | Acceptance idea | Source |
| --- | --- | --- | --- | --- | --- | --- | --- |
| PKG-001 | `.desktop`, icons, AppStream metainfo | P1 | 1 | packaging | None exist. AnyFlow has no menu entry, no icon and is invisible to GNOME Software / KDE Discover. Artwork already exists in `docs/design/assets/` | Menu entry with the Flowing Ribbon icon; the app appears in a software centre | [04 §11](04-LINUX-PORTABILITY.md), [07 §2](07-LINUX-PACKAGING.md) |
| PKG-002 | Package the GUI binary | P1 | 1 | packaging | The RPM ships only `anyflowd` and `anyflow` | `dnf install anyflow-gui` yields a working GUI | [07 §2](07-LINUX-PACKAGING.md) |
...
| PKG-005 | Split `anyflow` / `anyflow-gui` | P1 | 1 | packaging | The GTK floor excludes distributions from the GUI only; the daemon has no GTK dependency | Two binary packages in both RPM and DEB | [05 §7](05-DEBIAN-UBUNTU-COMPATIBILITY.md) |
...
| PKG-010 | Release tarball job | P2 | 2 | packaging | Covers every distribution AnyFlow does not package for, plus non-systemd | A signed tarball with checksums per release | [07 §3.3](07-LINUX-PACKAGING.md) |
...
| UX-007 | Show key backing | P2 | 5 | feature | Hardware vs software must be visible in `anyflow status`, the UI and first-run — never silent | The backing is displayed and logged at startup | [14 §7](14-CROSS-PLATFORM-IDENTITY-AND-KEY-STORAGE.md) |
